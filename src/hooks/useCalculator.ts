...
export function useCalculator() {
  ...
  const calculateResult = useCallback(() => {
    ...
    try {
      ...
      const historyItem: HistoryItem = {
        expression: expression ? `${expression} ${currentValue} =` : `${currentValue} =`,
  ...
  const handleBaseConversion = useCallback((toBase: string) => {
    if (error) {
      setError(null);
    }
  ...
  const parseBaseInput = useCallback((input: string, fromBase: string) => {
    if (error) {
      setError(null);
    }
